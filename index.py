...
def aide_allemands(md_fichier, html_fichier):
    ...
    .replace("ce", "ze").replace("c", "k").replace("ç", "z")
    .replace("ph", "f").replace("pp", "p").replace("gu", "ch")
    .replace("g", "ch").replace("j", "k").replace("v", "f")
    .replace("s", "").replace("mm","m"))
...
#    ajoutkikoo = html_fichier.append(["<p>Kikoo</p>", "<p>lol</p>", 
#    "<p>mdr</p>", "<p>ptdr</p>", "<p>lolilo</p>"])
...
parser.add_argument("-a", '--achtung',
            help='Convertir texte pour faciliter les allemands',
            action="store_true")
#parser.add_argument("-k", '--kikoo',help="ajoute 'kikoo', 'lol', 
# 'ptdr'... dans votre page html", action="store_true")
args = parser.parse_args()

#éxecution du convertisseur selon les options choisies
if args.achtung is True:
    print('Convertisseur .md en .html')
    print("Pour plus de renseignement sur l'utilisation du convertisseur,")
    print("veuillez lire le README")
